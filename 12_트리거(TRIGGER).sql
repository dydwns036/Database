/*
    <트리거>
    내가 지정한 테이블의 INSERT, UPDATE, DELETE등 DML문에 의한 변경사항이 생길 때
    (테이블에 이벤트가 발생했을 때)
    자동으로 매번 실행할 내용을 미리 정의해둘 수 있다.
    
    EX) 회원탈퇴시 기존의 회원테이블에 데이터 DELETE한 후 곧바로 탈퇴한 회원들만 따로 보관하는 테이블에
    자동으로 INSERT 시켜야한다.
    신고횟수가 일정 수를 넘었을 때 묵시적으로 해당 회원을 블랙리스트로 처리되겠끔
    입출고에 대한 데이터 기록(INSERT)될 때마다 해당 상품에 대한 재고 수량을 매번 수정(UPDATE)해야한다.
...
VALUES(SEQ_EID.NEXTVAL, '김홍시', '110111-1222222', 'D7', 'J7', SYSDATE);
...
-- 상품번호를 중복안되게끔 매번 새로운 번호를 발생시키는 시퀀스 생성
...
CREATE SEQUENCE SEQ_DECORE;

-- 200번 상품이 오늘잘짜로 10개 입고
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DECORE.NEXTVAL, 200, SYSDATE, 10, '입고');
...
-- 205번 상품이 오늘잘짜로 10개 입고
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DECORE.NEXTVAL, 205, SYSDATE, 20, '입고');
...
COMMIT;
-- TB_PRODETAIL 테이블에 INSERT이벤트 발생 시
-- TB_PRODUCT 테이블에 매번 자동으로 재고 수량을 UPDATE하게 끔 트리거 작성
...
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DECORE.NEXTVAL, 210, SYSDATE, 7, '출고');

-- 200번 상품이 오늘날짜로 100개 입고
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DECORE.NEXTVAL, 200, SYSDATE, 100, '입고');