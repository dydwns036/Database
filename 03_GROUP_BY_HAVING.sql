/*
    <GROUP BY절>
    그룹기준을 제시할 수 있는 구문(해당 그릅기준별로 여러 그룹으로 묶을 수 있음)
...
ORDER BY DEPT_CODE;                         -- 4 ORDER BY 무조건 마지막 실행 -> 정렬은 마지막 해주는 것이 아니면 의미 X

-- 각 직급별 총 사원 수, 보너스를 받는 사람 수, 급여합, 평균급여, 최저급여, 최고급여 (정렬 = 직급 오름차순)
SELECT JOB_CODE AS "직급", COUNT(JOB_CODE) AS "총 사원 수", 
       COUNT(BONUS) AS "보너스 받는 사람", SUM(SALARY) AS "급여합", 
       ROUND(AVG(SALARY)) AS "평균급여", MIN(SALARY) AS "최저급여", 
       MAX(SALARY) AS "최고급여"
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE;
...
-- 직급별 직급코드, 총급여합(단, 직급별 총급여함이 1000만원 이상인 직급만 조회
SELECT DEPT_CODE, SUM(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
/*
    ...
    - UNION : OR | 합집합(두 쿼리문의 수행한 결과값을 더한다)
    - INTERSECT : AND | 교집합(두 쿼리문의 수행한 결과값에 중복된 결과값)
    - UNION ALL : 합집합 + 교집합(중복되는 거 상관하지 않고 전부 더란 결과값)
    - MINUS : 차집합(선생 결과값에 후행 결과값을 뺀 나머지)
*/
...
-- 급여가 300만원 초과인 사원들의 사번, 이름, 부서코드, 급여조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY > 3000000;
...
-- 3. UNION ALL : 여러개의 쿼리 결과를 무조건 다 더한다(중복 상관 X)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION ALL
