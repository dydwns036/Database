...
SELECT EMP_NAME, TRUNC(ABS(HIRE_DATE - SYSDATE)) AS "근무일수1", FLOOR(SYSDATE - HIRE_DATE) AS "근무일수2"
FROM EMPLOYEE;
...
SELECT EMP_NAME, TO_CHAR(SALARY, 'L99,000,000')
FROM EMPLOYEE;

-- 21. EMPLOYEE테이블에서 직원 명, 부서코드, 생년월일, 나이(만) 조회
--     (단, 생년월일은 주민번호에서 추출해서 00년 00월 00일로 출력되게 하며
--     나이는 주민번호에서 출력해서 날짜데이터로 변환한 다음 계산)
SELECT EMP_NAME, DEPT_CODE, TO_CHAR(TO_DATE(SUBSTR(EMP_NO, 1, 6)), 'YY"년" MM"월" DD"일"'), 
...
--     (단, 부서코드 오름차순으로 정렬)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DECODE(DEPT_CODE, 'D5', '총무부', 'D6', '기획부', 'D9', '영업부') AS "부서코드"
FROM EMPLOYEE
...
--     주민번호 앞자리와 뒷자리의 합 조회
SELECT SUBSTR(EMP_NO, 1, 6) + SUBSTR(EMP_NO, 8)
FROM EMPLOYEE
WHERE EMP_ID = 201;
