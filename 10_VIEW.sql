...
-- 러시아에서 근무하는 사원들의 사번, 이름, 부서명, 급여, 근무국가명 조회SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
...
-- 일본에서 근무하는 사원들의 사번, 이름, 부서명, 급여, 근무국가명 조회SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
...
/*
    * DML 명령어로 조작이 불가능한 경우가 많다.
    ...
    6) JOIN을 이용하여 여러테이블을 연결시켜놓은 경우
    
    대부분은 뷰는 조회를 목적으로 생성한다. 그냥 뷰를 통한 DML은 안쓰는게 좋다.
*/
...
/*
    ...
    1) OR REPLACE : 기존에 동일한 뷰가 있을 경우 갱신하고, 없을 경우 새로 생성하라
    2) FORCE | NOFORCE
...
UPDATE VW_EMP SET SALARY = 2000000
WHERE EMP_ID = 200;
