...
/*
    ...
    FROM 테이블
    WHERE 조건식
    ORDER BY 정렬 기준될 컬럼 | 별칭 | 컬럼순번 | [ASC | DESC] [NULLS FIRST \ NULLS LAST]
    
    - ASC : 오름차순(작은 값으로 시작해서 값이 점점 커지는 것) -> 기본값
    - DESC : 내림차순(큰 값으로 시작해서 값이 점점 줄어드는 것)
    
    -- NULL은 기본적으로 가장 큰 값으로 분류해서 정렬한다.
    - NULL FIRST : 정렬하고자하는 컬럼 값에 NULL이 있을 경우 해당 데이터 맨 앞에 배치(DESC일 때, 기본값)
    - NULL LAST : 정렬하고자하는 컬럼 값에 NULL이 있을 경우 해당 데이터 맨 마지막에 배치(ASC일 때, 기본값)
*/
...
-- 정렬 기준에 컬럼 값이 동일할 경우 그 다음차순을 위해서 여러개를 제시할 수 있다.

-- 전 사원의 사원명, 연봉(보너스 제외) 조회(이 때 연봉별 내림차순 정렬)
SELECT EMP_NAME, (SALARY * 12) AS "연봉"
FROM EMPLOYEE
-- ORDER BY 연봉 DESC;
ORDER BY 2 DESC; -- 순번 사용 가능 오라클은 전부 1부터 시작

/*
    <함수 FUNCTION>
    전달된 컬럼 값을 받아서 함수를 실행한 결과를 반환
    
    - 단일행 함수 : N개의 값을 읽어들여서 N개의 결과 값을 리턴(매행마다 함수실행 결과를 반환)
    - 그룹 함수 : N개의 값을 읽어들여서 1개의 결과 값을 리턴(그룹을 지어서 그룹별로 함수실행 결과를 반환)

    >> SELECT 절에 단일행 함수랑 그룹 함수를 함께 사용하지 못함
    -> 결과 행의 갯수가 다르기 때문
...
/*
    *SUBSTR
    ...
    [표현법]
    SUBSTR(STRING, POSITION, [LEGNTH])
    - STRING : 문자타입 컬럼 | '문자열'
    - POSITION : 문자열 추출할 시작위치 값
    - LENGTH : 추출한 문자 갯수(생략하면 끝까지)
*/
...
SELECT  LTRIM('ACBABCAABCKKH') FROM DUAL;
SELECT  RTRIM('123456789KH987654321', '0123456789') FROM DUAL;
...
SELECT TRIM('Z' FROM 'ZZZZKHZZZZZZZZZZ') FROM DUAL; -- 양쪽에 있는 공백 제거

SELECT TRIM(LEADING 'Z' FROM 'ZZZZKHZZZZZZZZZZ') FROM DUAL; -- LTRIM
...
*/
SELECT EMAIL, REPLACE(EMAIL, 'KH.or.kr', 'gamil.com')
FROM EMPLOYEE;
--=============================================================================
/*
    <숫자 처리 함수>
...
-- BONUS가 있고, 여자인 사원
-- 아름, 주민번호, 직급코드, 부서코드, 급여, 보너스를 조회하고 싶다.
...
-- *ADD-MONTHS : 특정 날짜에 NUMBER개월수를 더해서 반환
...
/*
    *EXTRACT : 특정 날짜로부터 년|월|일 값을 추출해서 반환하는 함수
    ...
    EXTRACT(MONTH FROM DATE) : 월도만 추출
    EXTRACT(DAY FROM DATE) : 일도만 추출
*/
...
/*
    [형변환 함수]
    * TO_CHAR : 숫자타입 또는 날짜타입의 값을 문자타입으로 변환시켜주는함수
...
SELECT TO_CHAR(2000000, 'L9999990') FROM DUAL;  -- 현재 설정된 나라의 로컬 화폐단위 나타남
...
SELECT TO_CHAR(SYSDATE, 'PM HH:MI:SS') FROM DUAL;  -- AM, PM 어떤것을 사용하건 형식을 정해주는 것이기에 동일하게 나타남
...
SELECT TO_CHAR(SYSDATE, 'YYYY'),
        TO_CHAR(SYSDATE, 'YY'),
        TO_CHAR(SYSDATE, 'RRRR'),   -- RR룰이 따로 존재한다. -> 50년 이상값이 들어오면 + 100 EX) 1954 
...
-- COUNT(DISTINCT 칼럼) : 해당 컬럼값 중복을 제거한 후 행의 갯수를 세서 반환
