...
-- EMPLOYEE 테이블의 사원먕, 급여, 보너스, 연봉, 보너스 포함 연봉(급여 + (급여 * 보너스) * 12)
SELECT EMP_NAME, SALARY, BONUS, SALARY * 12, (SALARY + (SALARY * BONUS)) * 12 FROM EMPLOYEE;
-- 산술연산 과정 중에 NULL 데이터가 포함되어 있다면 무조건 결과값은 NULL

-- EMPLOYEE 테이블의 사원명, 입사일, 근무일수를 조회
-- 현재시간 - 입사일 = 근무한 기간
-- DATE - DATE => 결과는 무조건 일(DAY)로 표기됨
-- 코드실행 시, 현재 시간을 표시하는 상수 : SYSDATE
...
/*
    <컬럼명에 별칭 지정하기>
    산술연산을 하게되면 컬러명이 지저분해진다. 이때 컬럼명에 별칭을 부여해서 깔끔하게 가져올 수 있다.
    [표현법]
...
/*
    <DISTINCT>
    중복 제거 - 컬럼에 표시된 값들을 한 번씩만 조회하고자 할 때 사용
*/
...
-- 위처럼 작성 시, 에러가 발생한다. DISTINCT는 한 번만 사용 가능하다.
...
/*
    ...
    EX) 비교할 대상 컬럼 LIKE '문자%' : 비교할 대상 컬럼값 중에서 해당문자로 시작하는 값들만 가져온다.
        ...
        비교할 대상 컬럼 LIKE '%문자%' : 비교할 대상 컬럼값 중에서 해당문자로 포함된 값들만 가져온다.
        
    2. '_' : 1글자를 대체하는 검색
    EX) 비교할 대상 컬럼 LIKE '_문자' : 비교할 대상 칼럼값 문자앞에 아무 글자나 한 글자가 있는 값을 조회
        비교할 대상 컬럼 LIKE '문자_' : 비교할 대상 칼럼값 문자뒤에 아무 글자나 한 글자가 있는 값을 조회
        비교할 대상 컬럼 LIKE '_문자_' : 비교할 대상 칼럼값 문자앞뒤에 아무 글자나 한 글자가 있는 값을 조회
...
FROM EMPLOYEE
-- WHERE EMAIL LIKE '____' -> 와일드 카드 문자때문에 정상출력이 되지않는다.
-- 와일드카드문자와 일반문자를 구분해줘야한다.
-- 데이터 값으로 취급하고 싶은 와일드카드 문자앞에 나만의 탈출문자를 제시해서 탈출시켜주면된다.
...
-- 5. 보너스를 포함 연봉이 NULL이 아니고 이름에 '하'가 포함된 사원들의 사번, 사원명, 급여, 보너스 포함 연봉조회
