...
-- JOIN 절에 같이 조인하고자하는 테이블 기술 + 매칭시킬 컬럼에대한 조건
-- JOIN USING / JOIN ON

-- 1. 연결할 두 컬럼명이 다른 경우(EMLOYEE : DEPT_CODE / DEPARTMENT : DEPT_ID)
-- JOIN ON
-- 전체 사원들의 사번, 사원명, 부서코드 부서명
...
-- JOIN USING(연결하는 컬러명이 같은 경우만 가능)
...
/*
    ...
    단, 반드시 LEFT/RIGHT를 지정해야된다. (어떤 테이블 기준이냐)
*/
