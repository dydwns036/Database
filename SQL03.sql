...
-- 10.
SELECT STUDENT_NO AS "학번", STUDENT_NAME "학생 이름", ROUND(AVG(POINT), 1) AS "전체 평점"
FROM TB_STUDENT
JOIN TB_GRADE USING (STUDENT_NO)
JOIN TB_DEPARTMENT USING (DEPARTMENT_NO)
...
-- 16.
SELECT CLASS_NO, CLASS_NAME, TRUNC(AVG(POINT), 9) AS "AVG(POINT)"
FROM TB_CLASS
...
-- 19.
SELECT DEPARTMENT_NAME AS "계열 학과명", ROUND(AVG(POINT), 1) AS "평점"
FROM TB_DEPARTMENT
JOIN TB_STUDENT USING (DEPARTMENT_NO)
JOIN TB_GRADE USING (STUDENT_NO)
